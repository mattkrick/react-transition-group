  ...
  function createUnionTypeChecker(arrayOfTypeCheckers) {
    ...
    for (var i = 0; i < arrayOfTypeCheckers.length; i++) {
      ...
      if (typeof checker !== 'function') {
        process.env.NODE_ENV !== 'production' ? warning(false, 'Invalid argument supplied to oneOfType. Expected an array of check functions, but ' + 'received %s at index %s.', getPostfixForTypeWarning(checker), i) : void 0;
        return emptyFunction.thatReturnsNull;
      }
    }
    ...
    for (var typeSpecName in typeSpecs) {
      if (typeSpecs.hasOwnProperty(typeSpecName)) {
        ...
        try {
          ...
          !(typeof typeSpecs[typeSpecName] === 'function') ? process.env.NODE_ENV !== 'production' ? invariant(false, '%s: %s type `%s` is invalid; it must be a function, usually from ' + 'the `prop-types` package, but received `%s`.', componentName || 'React class', location, typeSpecName, _typeof(typeSpecs[typeSpecName])) : invariant(false) : void 0;
